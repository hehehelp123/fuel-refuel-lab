...
def stat(number, caption, color=None):
    ...
    return [
        {"t": number, "size": 40, "color": color or PURPLE, "bold": True, "space": 12},
        {"t": caption, "size": 13, "color": INK},
    ]
...
# ================================================================== задача
add(L_PURPLE, "Задача", [
    "На машине стоит датчик уровня топлива, блок шлёт показания раз "
    "в несколько секунд",
    "Надо находить моменты заправок",
...
add(L_CYAN, "Что приходит с машины", [
    ...
    "Сообщения неравномерные: от секунды до нескольких минут",
    "Уровень — в единицах датчика, не в литрах",
    "Перевод в литры — по тарировочной таблице своей машины",
])

add(L_PURPLE, "Порог на приросте не работает", [
    "На стоянке датчик не передаёт ничего",
    "На ходу уровень гуляет от разгонов и уклонов",
    "Низкий порог ловит колебания, высокий — пропускает заправки",
] + stat("0.37", "событийный F1 лучшего порога"))

add_two_pics("Зажигание делит задачу надвое", L_CYAN,
             ("Двигатель заглушен", "s_ign_off.png",
              "Роста нет: 8 л до остановки, 60 л после запуска. 711 событий"),
             ("Двигатель работает", "s_event.png",
              "Уровень растёт 10–20 минут, рост виден. 538 событий"))
...
add(L_CYAN, "Модель", stat("45 с", "обучение на процессоре") + [
...
add_two("Точечная метрика обманывает", layout_idx=L_PURPLE, left=
        ("Угадала 3 интервала из 5", [
            "Точечная: средне",
            "Событийная: заправка найдена",
        ]),
        right=("Одно срабатывание на стоянке", [
            "Точечная: почти незаметно",
            "Событийная: ложная тревога",
        ]))

add(L_CYAN, "Событийная метрика", [
    {"t": "Событие — непрерывный блок класса 1. Засчитывается при пересечении "
          "с эталоном ±5 минут, это один интервал.", "size": 15},
], body_box=(0.75, 4.3, 8.5, 0.7),
    picture="s_event_metric.png", pic_box=(1.7, 1.5, 6.6))

add(L_PURPLE, "Результаты", stat("0.853", "событийный F1") + [
...
], body_box=(4.4, 1.5, 4.8, 3.2),
    picture="s_confusion.png", pic_box=(0.8, 1.45, 3.2))

add(L_CYAN, "Модель против порога", [
    {"t": "Порог перебран от 1 до 20 литров", "size": 15},
    {"t": "У порога precision около 0.22: находит заправки вместе "
          "с колебаниями", "size": 15, "space": 8},
    {"t": "Модель смотрит на форму окна, а не на одно число", "size": 15,
     "space": 8},
], body_box=(5.9, 1.8, 3.4, 2.6),
    picture="s_compare.png", pic_box=(0.7, 1.9, 4.9))

add(L_PURPLE, "Задание", [
    {"t": "Поднять событийный F1 на тестовых машинах", "size": 19, "bold": True},
] + stat("0.88", "планка «хорошо»") + [
    {"t": "Признаки · нормализация · длина окна · веса классов · архитектура · "
          "сглаживание · порог вероятности · размер интервала",
     "size": 14, "space": 14},
])

add(L_FINAL, "Вопрос со звёздочкой", [
    {"t": "Модель для заправок на стоянке даёт accuracy 100% и F1 = 1.0",
     "size": 16},
    {"t": "Метка ставится по правилу: три точки, в середине ноль, по краям "
          "уровни стабильны и различаются. Признаки — уровень и его разности.",
     "size": 15, "space": 12},
    {"t": "Почему это ничего не говорит о качестве детекции?",
     "size": 19, "bold": True, "color": RED, "space": 16},
])
