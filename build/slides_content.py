...
def build(ctx):
    ...
    text_slide_with_table(
        ...
        note="Одна строка — 5 минут одной машины. Разбиение по машинам: "
             ...
             "на ходу, 2 — на стоянке.",
        col_w=[1.2, 1.8, 1.3, 1.1, 1.3, 1.6], size=11)

    # --------------------------------------------------------- дисбаланс
    add(L, "Целевой класс редкий", stat("1.8%", "интервалов — заправка на ходу") + [
    ...
    add(L, "Считаем события, а не точки", [
        {"t": "Угадала 3 интервала из 5 — событие всё равно найдено. "
              "Одно срабатывание на стоянке — ложная тревога.", "size": 14},
    ...
    # ---------------------------------------------------------- результаты
    add(L, "Результаты", stat("0.853", "событийный F1") + [
