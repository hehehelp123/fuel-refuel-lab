...
def build(ctx):
    ...
    add(L, "Считаем события, а не точки", [
        {"t": "Серые блоки — заправки по разметке эксперта, фиолетовые — что нашла модель.",
         "size": 14},
        {"t": "Границы почти никогда не совпадают точь-в-точь, поэтому событие "
              "засчитывается с допуском ±5 минут — это один интервал.",
         "size": 14, "space": 8},
    ], body_box=(0.8, 4.15, 8.4, 1.0),
        picture="s_event_metric.png", pic_box=(1.6, 1.45, 6.8))
    ...
    ], body_box=(0.85, 3.92, 8.4, 1.25),
        picture="s_leak.png", pic_box=(2.4, 1.32, 5.2))
