...
ax.set_title("Бейзлайн на тестовых машинах")
ax.grid(False)
plt.tight_layout()
...
bars = ax.bar(["пороговое правило", "LSTM"], [best_thr_f1, lstm_f1],
              color=["#b0b0b0", "#1a1a1a"], width=0.55)
...
ax.set_title("Что даёт модель поверх простого порога")
plt.tight_layout()
...
ax.plot(w.ts, w.fuel, marker="o", ms=4, lw=1.5, color="#1a1a1a")
ax.axvspan(g.ts[s], g.ts[e], color="#c8c8c8", alpha=0.6, label="размеченная заправка")
ax.xaxis.set_major_formatter(matplotlib.dates.DateFormatter("%H:%M"))
ax.set_ylabel("литры")
ax.set_xlabel(f"{top}, {g.ts[s]:%d.%m.%Y}")
ax.legend()
ax.set_title("Как выглядит заправка при работающем двигателе")
plt.tight_layout()
