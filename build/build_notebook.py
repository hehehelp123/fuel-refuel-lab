...
(июль–декабрь 2025), 6.1 млн исходных сообщений, свёрнутых в **5-минутные бины**.
...
| `n_msg` | сколько исходных сообщений попало в бин |
...
остаётся только скачок уровня между двумя соседними бинами. Мы его не выкидываем,
...
print("Распределение меток по бинам:")
...
for name, part in df.groupby("split"):
    ...
    print(f"{name:5s}: машин {part.vehicle.nunique():2d}, бинов {len(part):6d}, событий класса 1: {ev}")''')
...
md("""Видно главное: заправка — это **монотонный рост уровня на протяжении нескольких бинов**.
Ровно это модель и должна научиться отличать от плеска и от шума датчика.

Обратите внимание на нули в `fuel`: это бины, где датчик молчал (двигатель заглушен).
...
Признаки на один бин — намеренно простые:

* `fuel` — сам уровень;
* `d1..d4` — разности с уровнем на 1, 2, 3 и 4 бина назад.

Зачем четыре разности разной глубины: `d1` ловит резкий скачок между соседними бинами,
...
Дальше нарезаем **скользящие окна по 7 бинов** (35 минут). Класс окна — это класс
его **центрального** бина. Центр, а не конец, потому что для решения «была ли тут заправка»
одинаково полезен контекст и до, и после момента.
""")

code('''FEATURES = ["fuel", "d1", "d2", "d3", "d4"]
SEQ_LEN = 7          # длина окна в бинах
CENTER = SEQ_LEN // 2
...
Целевой класс занимает меньше 2% бинов. Если ничего не делать, модель выучит
...
md("""Класс 2 в тесте представлен всего парой десятков бинов, поэтому его строчка
...
Разница существенная. Модель, которая на каждой заправке угадала 3 бина из 5,
по точечной метрике выглядит средне, а по событийной — идеально: событие найдено.
И наоборот, модель, которая ровно один бин посреди пустой стоянки пометила заправкой,
...
чем на X литров за бин — значит заправка». Посмотрим, что он даёт на тех же данных.
Это наша нижняя планка: модель, которая не бьёт порог, не нужна.
""")

code('''# прирост уровня в центральном бине окна — то, на что смотрит пороговое правило
...
* **Длина окна.** 7 бинов = 35 минут. Длиннее — больше контекста и меньше окон; короче — наоборот.
...
* **Постобработка.** Одиночный бин класса 1 посреди нулей — почти наверняка ложное
...
* **Другой размер бина.** Файл `fuel_raw.csv.gz` содержит исходные сообщения.
